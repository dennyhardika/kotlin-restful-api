...
class OrderfdServiceImpl(val orderfdRepository: OrderfdRepository, val orderRepository: OrderRepository, val fileStorageService: FileStorageService): OrderfdService {
    ...
    private fun convertOrderfdToOrderfdResponse(ordersfd: Ordersfd): OrderfdResponse {
        return OrderfdResponse(
            ...
            foto_onefd = if (!ordersfd.foto_onefd.isNullOrEmpty()) fileStorageService.generateFileUrl(ordersfd.foto_onefd!!) else "",
            foto_twofd = if (!ordersfd.foto_twofd.isNullOrEmpty()) fileStorageService.generateFileUrl(ordersfd.foto_twofd!!) else "",
            foto_threefd = if (!ordersfd.foto_threefd.isNullOrEmpty()) fileStorageService.generateFileUrl(ordersfd.foto_threefd!!) else "",
            foto_fourfd = if (!ordersfd.foto_fourfd.isNullOrEmpty()) fileStorageService.generateFileUrl(ordersfd.foto_fourfd!!) else "",
